...
public class TopicInfoFragment extends Fragment implements MenuProvider, MessageActivity.DataSetChangeListener {
    ...
    public void onViewCreated(@NonNull View view, Bundle savedInstanceState) {
        ...
        View avatar = view.findViewById(R.id.imageAvatar); // ha nincs ilyen id,
        // próbáld: R.id.avatar vagy R.id.avatarImage
        if (avatar != null) {
            avatar.setOnClickListener(v -> {
                // MessageActivity tudja a startAvatarSelection()-t
                if (requireActivity() instanceof MessageActivity) {
                    ((MessageActivity) requireActivity()).startAvatarSelection();
                }
            });
        }

...
public class MessageActivity extends AppCompatActivity
    ...
    // ===== AVATAR PERMISSION & PICKER (ÚJ) =====
    private final ActivityResultLauncher<String> mImagePermissionLauncher =
            registerForActivityResult(new ActivityResultContracts.RequestPermission(), isGranted -> {
                if (isGranted) {
                    launchAvatarImagePicker();
                } else {
                    Toast.makeText(this,
                            "Engedély szükséges a profilkép kiválasztásához.", Toast.LENGTH_SHORT).show();
                }
            });

    private final ActivityResultLauncher<String> mPickAvatarImageLauncher =
            registerForActivityResult(new ActivityResultContracts.GetContent(), uri -> {
                if (uri == null) return;
                // Megnyitjuk az Avatar Preview fragmentet (beépített a projektben)
                ...
                showFragment(FRAGMENT_AVATAR_PREVIEW, args, true);
            });
    // ===========================================
    ...
    // Bal-alsó "Éppen ír…" jelzés automatikus elrejtése
    private final Runnable hideTypingRunnable = new Runnable() {
        @Override
        public void run() {
            MessagesFragment frag = (MessagesFragment) getSupportFragmentManager()
                    .findFragmentByTag(FRAGMENT_MESSAGES);
            if (frag != null && frag.getView() != null) {
                View indicator = frag.getView().findViewById(R.id.typing_indicator);
    ...
    private static final int REQ_CALL_PERMS = 2001;

    private boolean ensureCallPermissions(boolean video) {
        List<String> need = new java.util.ArrayList<>();
        need.add(Manifest.permission.RECORD_AUDIO);
        if (video) need.add(Manifest.permission.CAMERA);
        ...
        if (!missing.isEmpty()) {
            androidx.core.app.ActivityCompat.requestPermissions(
                    this, missing.toArray(new String[0]), REQ_CALL_PERMS);
            return false;
        }
        return true;
    }

    // ===== AVATAR INDÍTÓ (ÚJ, publikus) =====
    public void startAvatarSelection() {
        // Android 13+ (API 33): READ_MEDIA_IMAGES, korábban READ_EXTERNAL_STORAGE
        ...
        if (androidx.core.content.ContextCompat.checkSelfPermission(this, perm)
                != android.content.pm.PackageManager.PERMISSION_GRANTED) {
            mImagePermissionLauncher.launch(perm);
        } else {
            launchAvatarImagePicker();
        }
    }

    private void launchAvatarImagePicker() {
        // Csak képeket kérünk
        mPickAvatarImageLauncher.launch("image/*");
    }
    // ========================================
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        ...
        if (id == R.id.action_view_contact) {
            ...
            if (id == R.id.action_archive) {
                ...
                boolean audioOnly = (id == R.id.action_audio_call);
                if (!ensureCallPermissions(!audioOnly)) {
                    return true;
                }
                CallManager.placeOutgoingCall(this, mTopicName, audioOnly);
                return true;
            }
        } else {
    ...
    private class TListener extends ComTopic.ComListener<VxCard> {
        ...
        public void onData(MsgServerData data) {
            // Saját üzenetre ne küldjünk read-et
            if (data != null && !Cache.getTinode().isMe(data.from)) {
                sendNoteRead(data.seq);
            }

            // Bejövő üzenet → rejtsük el a "Éppen ír…" jelzést azonnal
            runOnUiThread(() -> {
                MessagesFragment frag = (MessagesFragment) getSupportFragmentManager()
                        .findFragmentByTag(FRAGMENT_MESSAGES);
                if (frag != null && frag.getView() != null) {
                    View indicator = frag.getView().findViewById(R.id.typing_indicator);
                    if (indicator != null) {
                        indicator.removeCallbacks(hideTypingRunnable);
                    ...
                    runOnUiThread(() -> {
                        ...
                        if (frag != null && frag.getView() != null) {
                            ...
                            if (indicator != null) {
                                ...
                                // timeout reset
                                indicator.removeCallbacks(hideTypingRunnable);
                                indicator.postDelayed(hideTypingRunnable, TYPING_INDICATOR_DURATION);
                            }
                        }
                    });
                    break;
